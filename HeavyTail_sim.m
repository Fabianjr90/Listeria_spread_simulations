% Size of hop is sampled from polynomial 1/u^alpha
...
d = 1.0;                              % hop scaling factor
delt = 0.01;                          % timestep
krep = 1.0;                           % replication rate
dTime = log(2)/krep;                  % doubling time
sigma = dTime/5;                      % width of dTime distribution
alpha = 3.4;                          % heavy tail exponent
...
for step = 1:maxstep
    ...
    delxy = levySample(d*delt,alpha,nlive);
...
g = fit(time(1:(step-1)),bacteria_number(1:(step-1)),'exp1');
graphExpFit(time,bacteria_number,g,step);
k_guess = g.b;

% calculating slope of <r^2>
p = polyfit(time(1:(step-1)),avgR2(1:(step-1)),1);
